...
def count_hits_learned_full(rows, tau, direction, backup_ratio=0.70, rho3=8, backup_margin=4):
    hits = 0
    for r in rows:
        score = float(r["score"])
        freq = int(r["freq"])
        ...
        if direction == "high_is_trending":
            near_threshold = score > (tau * backup_ratio)
        else:
            near_threshold = score < (tau + (1 - tau) * (1 - backup_ratio))

        if near_threshold and freq >= (rho3 + backup_margin):
            hits += 1
    return hits
...
def compute_filter_fpr(
    ...
    for item_id, rows in by_item.items():
        ...
        lfull_hits = count_hits_learned_full(
            rows, tau=tau, direction=direction, backup_ratio=backup_ratio, rho3=rho3, backup_margin=backup_margin
        )
...
class DetectionSketch:
    ...
    def _admit(self, score, freq):
        ...
        if self.filter_mode == "learned_primary":
            return score_pass(score, self.tau, self.direction)
        if self.filter_mode == "learned_full":
            if score_pass(score, self.tau, self.direction):
                return True
            if self.direction == "high_is_trending":
                near_threshold = score > (self.tau * self.backup_ratio)
            else:
                near_threshold = score < (self.tau + (1 - self.tau) * (1 - self.backup_ratio))
            return near_threshold and freq >= (self.rho3 + self.backup_margin)
...
def compute_detector_occupancy(
    ...
    standard = DetectionSketch(memory_kb, "standard", tau, direction, backup_ratio, backup_margin, rho3)
    learned_primary = DetectionSketch(memory_kb, "learned_primary", tau, direction, backup_ratio, backup_margin, rho3)
    learned_full = DetectionSketch(memory_kb, "learned_full", tau, direction, backup_ratio, backup_margin, rho3)
