...
def count_hits_learned_full(rows, tau=0.30, backup_ratio=0.85, rho3=8):
    hits = 0
    for r in rows:
        score = float(r["score"])
        freq = int(r["freq"])
        if score > tau:
            hits += 1
        elif score > (tau * backup_ratio) and freq >= rho3:
            hits += 1
    return hits
...
def compute_filter_fpr(
    ...
    for item_id, rows in by_item.items():
        ...
        lfull_hits = count_hits_learned_full(rows, tau=tau, backup_ratio=backup_ratio, rho3=rho3)
