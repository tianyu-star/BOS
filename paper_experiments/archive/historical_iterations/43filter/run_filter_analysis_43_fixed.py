...
def learned_primary_admit(rows, tau=0.30):
    return any(float(r["score"]) > tau for r in rows)

def learned_full_admit(rows, tau=0.30, backup_ratio=0.85, rho3=8):
    for r in rows:
        score = float(r["score"])
        freq = int(r["freq"])
        if score > tau:
...
def compute_filter_fpr(records, memory_kb, tau=0.30, backup_ratio=0.85, base_rho3=8, ref_memory_kb=10, label_field="label"):
    ...
    for item_id, rows in by_item.items():
        ...
        if learned_primary_admit(rows, tau=tau):
            learned_primary.add(item_id)
        if learned_full_admit(rows, tau=tau, backup_ratio=backup_ratio, rho3=rho3):
...
def compute_detector_occupancy(records, memory_kb=10, tau=0.30, backup_ratio=0.85, base_rho3=8, ref_memory_kb=10, label_field="label"):
    ...
    standard = DetectionSketch(memory_kb=memory_kb, filter_mode="standard", tau=tau, backup_ratio=backup_ratio, rho3=rho3)
    learned_primary = DetectionSketch(memory_kb=memory_kb, filter_mode="learned_primary", tau=tau, backup_ratio=backup_ratio, rho3=rho3)
    learned_full = DetectionSketch(memory_kb=memory_kb, filter_mode="learned_full", tau=tau, backup_ratio=backup_ratio, rho3=rho3)
...
def main():
    ...
    out_rows = []
    for mem in budgets:
        out_rows.append(compute_filter_fpr(
            records,
            memory_kb=mem,
            tau=args.tau,
