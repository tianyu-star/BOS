...
def count_hits_learned_primary(rows, tau=0.30):
    return sum(1 for r in rows if float(r["score"]) > tau)


def count_hits_learned_full(rows, tau=0.30, backup_ratio=0.75, rho3=8, backup_margin=2):
    hits = 0
    for r in rows:
        score = float(r["score"])
        freq = int(r["freq"])
        if score > tau:
            hits += 1
        elif score > (tau * backup_ratio) and freq >= (rho3 + backup_margin):
            hits += 1
    return hits
...
def auto_tau_from_scores(records, label_field="label", quantile=0.90, fallback=0.295):
    neg_scores = [float(r["score"]) for r in records if int(r.get(label_field, 0)) == 0]
    if not neg_scores:
        return fallback
    neg_scores.sort()
    idx = min(len(neg_scores) - 1, max(0, round((len(neg_scores) - 1) * quantile)))
    return neg_scores[idx]
...
def compute_filter_fpr(
    ...
    for item_id, rows in by_item.items():
        ...
        lpri_hits = count_hits_learned_primary(rows, tau=tau)
        lfull_hits = count_hits_learned_full(
            rows,
            tau=tau,
...
class DetectionSketch:
    ...
    def _admit(self, score, freq):
        ...
        if self.filter_mode == "learned_full":
            if score > self.tau:
                return True
            if score > (self.tau * self.backup_ratio) and freq >= (self.rho3 + self.backup_margin):
                return True
            return False
...
def compute_detector_occupancy(
    ...
    learned_primary = DetectionSketch(
        memory_kb=memory_kb,
        filter_mode="learned_primary",
        tau=tau,
        backup_ratio=backup_ratio,
        backup_margin=backup_margin,
        rho3=rho3,
    )
    learned_full = DetectionSketch(
        memory_kb=memory_kb,
        filter_mode="learned_full",
        tau=tau,
        backup_ratio=backup_ratio,
        backup_margin=backup_margin,
        rho3=rho3,
...
def main():
    ...
    parser.add_argument("--backup-ratio", type=float, default=0.75)
    parser.add_argument("--backup-margin", type=int, default=2)
    ...
    tau_used = args.tau
    if args.auto_tau:
        tau_used = auto_tau_from_scores(
            records,
            label_field=args.label_field,
            quantile=args.neg_quantile,
            fallback=args.tau,
        )
