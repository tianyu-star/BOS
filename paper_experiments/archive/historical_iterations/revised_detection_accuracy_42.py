...
"""
Improved Figure 2 runner for the current window-level proxy pipeline.

What this version changes versus the earlier draft:
1) Evaluates at item level by default.
2) Exposes a "relaxed" parameter preset for short traces with only 3-10 windows.
3) Tracks detector admissions and reports how many items ever entered the detector.
4) Keeps the paper-style variants:
   - scout_manual
   - scout_relaxed
   - scout_grid
   - neutrend_filter
...
This is still an approximation of Section 4.2 because the score is not the paper's
original BNN score s_e. It is meant to make the pipeline diagnosable and tunable.
"""
...
def make_variant(memory_kb, variant, tau):
    ...
    if variant == "neutrend_adaptive":
        return DetectionSketch(
            ...
            Gmin=1.03, Gmax=1.25,
            Dmin=0.70, Dmax=0.95,
            rho1_min=2, rho1_max=6,
            rho2_min=2, rho2_max=4,
            alpha_rho=0.5, rho3_min=4,
            beta_rho=0.8,
            alpha_rho_damp=0.5, rho6_min=8,
        )
...
def run_variant(records, memory_kb, variant, tau=0.5, label_field="label"):
    sketch = make_variant(memory_kb, variant, tau)
    ...
    metrics["num_seen_items"] = len(sketch.ever_seen_items)
    return metrics


def grid_search_scout(records, memory_kb, label_field="label"):
    grids = {
        "G": [1.05, 1.10, 1.20],
        "D": [0.70, 0.80, 0.90],
        "rho1": [2, 3, 5],
    ...
    for vals in itertools.product(*grids.values()):
        ...
        if best is None or metrics["f1"] > best["f1"]:
            best = dict(metrics)
            best["config"] = cfg
            best["num_admitted_items"] = len(sketch.admitted_items)
            best["num_pred_items"] = len(sketch.predicted_items)
    return best
...
def main():
    ...
    parser.add_argument("--grid-budget", type=int, default=10)
    ...
    for mem in budgets:
        row = {
            "memory_kb": mem,
            "scout_manual": run_variant(records, mem, "scout_manual", tau=args.tau, label_field=args.label_field),
            "scout_relaxed": run_variant(records, mem, "scout_relaxed", tau=args.tau, label_field=args.label_field),
            "neutrend_filter": run_variant(records, mem, "neutrend_filter", tau=args.tau, label_field=args.label_field),
            "neutrend_adaptive": run_variant(records, mem, "neutrend_adaptive", tau=args.tau, label_field=args.label_field),
            "neutrend_full": run_variant(records, mem, "neutrend_full", tau=args.tau, label_field=args.label_field),
        }
        if mem == args.grid_budget:
            row["scout_grid"] = grid_search_scout(records, mem, label_field=args.label_field)
        results.append(row)

    with open(args.out_json, "w", encoding="utf-8") as f:
        json.dump(results, f, ensure_ascii=False, indent=2)

    print(f"Saved improved Figure 2 data to {args.out_json}")
